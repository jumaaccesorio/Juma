  ...
  return (
    <div className="flex-1 min-h-screen space-y-20 bg-secondary p-6 dark:bg-carbon md:p-10">
      ...
      <div className="rounded-xl border border-neutral-soft bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Carga un CSV exportado desde Excel o Google Sheets con estas columnas: Nombre, subnombre,
              precio_compra, precio_venta, stock, categoria.
            </p>
            <p className="mt-2 text-xs uppercase tracking-wider text-slate-400">
              Si Nombre viene vacio, la tienda mostrara el subnombre.
            </p>
          </div>
          <label className="inline-flex cursor-pointer items-center justify-center rounded-lg bg-primary px-5 py-3 text-sm font-bold text-white shadow-lg shadow-primary/20 transition-all hover:bg-primary/90">
        ...
        {importMessage ? (
          <div className="mt-4 rounded-lg bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700">
            {importMessage}
          </div>
        ) : null}
      </div>
      ...
      {showForm && (
        <form
          className="animate-fade-in rounded-xl border border-neutral-soft bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900"
          onSubmit={(event) => {
            onAddProduct(event);
            setShowForm(false);
          }}
        >
          ...
          <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Ej. Collar Corazon Plata"
          ...
          <div className="mb-8 flex items-center gap-6 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-4">
            ...
            <div className="flex-1">
              ...
              <input
                type="file"
                accept="image/*"
                className="text-sm text-slate-500 file:mr-4 file:rounded-full file:border-0 file:bg-primary/10 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-primary hover:file:bg-primary/20"
          ...
          <div className="flex justify-end gap-4">
            <button
              ...
              className="rounded-lg px-6 py-3 font-bold text-slate-600 transition-colors hover:bg-slate-100"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="rounded-lg bg-primary px-8 py-3 font-bold text-white shadow-lg shadow-primary/20 transition-all hover:bg-primary/90"
            >
              Guardar Producto
            </button>
          </div>
      ...
      <div className="overflow-hidden rounded-xl border border-neutral-soft bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col items-center justify-between gap-4 border-b border-neutral-soft p-4 dark:border-slate-800 sm:flex-row">
          <div className="flex w-full gap-4 overflow-x-auto sm:w-auto">
            ...
            <button className="whitespace-nowrap px-4 py-2 text-sm font-medium text-slate-500 transition-colors hover:text-slate-700">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 pl-10 pr-4 text-sm outline-none transition-all focus:ring-2 focus:ring-primary/20"
              placeholder="Buscar..."
              value={query}
              ...
              {filteredProducts.map((product) => (
                <tr key={product.id} className="transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                  ...
                  <td className="whitespace-nowrap p-4 text-sm font-medium text-slate-600 dark:text-slate-400">
                    ...
                    {product.isFeatured && (
                      <span
                        className="ml-2 rounded bg-yellow-100 px-1.5 py-0.5 text-[10px] font-bold uppercase text-yellow-700"
                  ...
                  <td className="p-4">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        {product.stock <= 2 && (
                          <span className="rounded bg-red-100 px-2 py-0.5 text-[10px] font-black uppercase text-red-700">Low</span>
                        )}
                      </div>
                  ...
                  <td className="p-4">
                    <label className="relative inline-flex cursor-pointer items-center">
                      ...
                      />
                      <div className="peer h-6 w-11 rounded-full bg-slate-200 peer-checked:bg-primary peer-focus:outline-none peer-checked:after:translate-x-full peer-checked:after:border-white after:absolute after:start-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-['']"></div>
                      <span className="ms-3 w-12 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">
